...
public class Heap<AnyType extends Comparable<AnyType>>
{
  ...
  */
   ...
   private void percolatingDown(int k) //topdown 
   {
      AnyType tmp = heap[k];
      int child;
      ...
      for(; 2*k <= size; k = child)
      {
         ...
         if(child != size &&
            heap[child].compareTo(heap[child + 1]) > 0) child++;//find min child 
   ...
   private void bottomUp(int last){
	   AnyType tmp=heap[last];
	   int parentPosition=last;
	   int child; 
	   //for minimum heap
	   while (parentPosition>=1){
		   parentPosition=last/2;
		   child=parentPosition*2;
		   
		   if(child != size &&
		            heap[child].compareTo(heap[child + 1]) > 0) child++;//find min child 
		   
		   if (heap[parentPosition].compareTo(heap[child])>0) {
			   AnyType temp=heap[child];
			   heap[child]=heap[parentPosition];
